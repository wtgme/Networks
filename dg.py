...
#plt.title('Numbers of Nodes with Degree K Plot(reply)')
...
plt.title('Cumulative Distribution of Nodes with Degree K Plot(reply)')
plt.ylabel('P')
plt.ylim(0.0,1.0)
plt.xlabel('Degree')
degseq=list(DG.degree(weight='weight').values())
...
for d in degseq:
    freq[d] += 1
#plt.plot(freq)
plt.plot(list(float(nx.utils.cumulative_sum(freq))/sum(freq)))
plt.show()
